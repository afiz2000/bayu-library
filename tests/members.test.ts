...
describe("members API", () => {
  it("creates PERSON+MEMBER atomically (server-assigned IDs), updates, and deletes both", async () => {
    ...
    const deleteRes = await deleteMember(makeRequest("DELETE", `/api/members/${memberId}`), ctx(memberId));
  ...
  it("returns a friendly error (not raw ORA-) for a duplicate email, without retrying the ID", async () => {
    ...
    await deleteMember(makeRequest("DELETE", `/api/members/${firstBody.data.member_id}`), ctx(firstBody.data.member_id));
  });
});
